...
class KeyValueStoreServicer(pb2_grpc.KeyValueStoreServicer):
    ...
    def List(self, request: pb2.ListRequest, context: grpc.ServicerContext) -> pb2.ListResponse:
        items = []
    ...
    def Get(self, request: pb2.GetRequest, context: grpc.ServicerContext) -> pb2.GetResponse:
        try:
    ...
    def Put(self, request: pb2.PutRequest, context: grpc.ServicerContext) -> pb2.PutResponse:
        ...
        if len(self.storage) > 10:
            del self.storage[min(self.storage, key=lambda key: self.storage[key]["last_used"])]
        return pb2.PutResponse()
    
    def Delete(self, request: pb2.DeleteRequest, context: grpc.ServicerContext) -> pb2.DeleteResponse:
        try:
            obj = self.storage[request.key]
            del self.storage[request.key]
